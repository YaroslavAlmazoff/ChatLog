...
const FileStorage = () => {
    ...
    useEffect(() => {
        ...
        const getFiles =  async () => {
            ...
            const response = await api.post(`/api/cloud/files`, {folder: JSON.stringify(currentFolder)}, {headers: {
    ...
    return (
        <div className='file-storage'>
            {!filesLoading ? <Files
                ...
                file={selectedFile}
            /> : <Loader ml={'50%'} />}
            <div className='file-detail-wrapper'>{JSON.stringify(selectedFile) !== '{}' ? <FileDetail 
                setFiles={setFiles} 
