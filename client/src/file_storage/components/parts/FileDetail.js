...
const FileDetail = ({file, detailDisplay}) => {
    ...
    const deleteFile = async () => {
        await api.get(`/api/cloud/delete/${file._id}`, {headers: {
    ...
    return (
        <div className="file-detail" style={{display: detailDisplay}}>
            ...
            {file.ext === 'txt' || file.ext === 'jpg' || file.ext === 'jpeg' || file.ext === 'png' || file.ext === 'gif' || file.ext === 'mp3' || file.ext === 'mp4' ?  <button onClick={sendFileLink} className='button'>Отправить ссылку на файл</button> : null }
                <button onClick={sendFile} className='button'>Отправить файл</button>
                {file.ext === 'txt' || file.ext === 'jpg' || file.ext === 'jpeg' || file.ext === 'png' || file.ext === 'gif' || file.ext === 'mp3' || file.ext === 'mp4' ? <><button onClick={() => setPreviewOpened(true)} className='button'>Предпросмотр файла</button>
                <button onClick={openFile} className='button'>Открыть файл</button></> : null}
                <a className='button' href={process.env.REACT_APP_API_URL + '/' + file.path.split('/').slice(4).join('/')} download={file.name}>Скачать</a>
                <button onClick={deleteFile} className='button'>Удалить</button>
            </div>
            {previewOpened && <FilePreview file={file} ready={false} fileOpened={false} />}
            <LinkRecipientsList file={file} linkRecipientsDisplay={linkRecipientsDisplay} />
            <RecipientsList file={file} recipientsDisplay={recipientsDisplay} setRecipientsDisplay={setRecipientsDisplay} />
        </div>
    )
}
