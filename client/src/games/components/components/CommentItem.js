...
const GameCommentItem = ({item}) => {
    ...
    const like = async () => {
        if(!isLike && localStorage.getItem(item._id) !== auth.userId) {
            ...
            setLikesCount(likesCount - 1)
            setIsLike(false)
            const response = await api.get(`/api/games/dislikecomment/${item._id}`)
            console.log(response)
  ...
  return (
    <div className={`videohost-comment-item ${randomBlockShadow()}`} style={window.innerWidth < 500 ? {width: '92%'} : {width: '60%'}}>
        <div className="videohost-comment-item-avatar">
            <img className={`videohost-comment-item-avatar-img ${randomBlockShadow()}`} src={process.env.REACT_APP_API_URL + '/useravatars/' + user.avatarUrl} alt="" />
            <div onClick={like}>
        ...
        <div className="videohost-comment-item-main">
            <div className="videohost-comment-item-head">
                <span className={`videohost-comment-item-author ${randomColor()} ${randomShadow()}`}>{user.name} {user.surname}</span>
