...
const Game = () => {
    ...
    useEffect(() => {
        ...
        const getComments = async () => {
            const response = await api.get(`/api/games/comments/${params.id}`)
    ...
    return (
        <div className="game-overlay">
            {window.innerWidth > 500 ? <div className="game block">
                <div className="game-top">
                    <div>
                        <img className={`game-top-preview ${randomBlockShadow()}`} src={process.env.REACT_APP_API_URL + '/gamepreviews/' + game.previewUrl} alt="" />
                ...
                {game.description && window.innerWidth < 500 ? <p className={`game-description ${randomShadow()}`}>{game.description}</p> : <></>}
            ...
            <div className="game block">
                ...
                <div className="game-mobile-info-info">
                    ...
                    <p className={`game-description ${randomShadow()}`}>{game.description}</p>
                </div>
                {game.rating && <Rating id={game._id} rating={game.rating} />}
                <CommentField setComments={setComments} />
                {comments.map(item => <CommentItem item={item} />)}
