...
const CreatePost = () => {
    ...
    const getFile = async (e) => {
        ...
        if(fileList.length > 5) {
            alert('Слишком много загружаемых файлов. Попробуйте выбрать меньше пяти файлов.')
            return
        }
    ...
    const send = async () => {
        ...
        await api.post(`/api/createuserpost`, formData, {headers: 
    ...
    return (
        <>
        {!loading ? <div className="create-post block">
            <input onChange={(e) => getFile(e)} ref={fileRef} type="file" accept=".jpg, .png, .gif" multiple />
            ...
            <button onClick={(e) => emitOpen(e)} className="button">Выбрать фото</button>
            <button onClick={send} className="button">Добавить запись</button>
