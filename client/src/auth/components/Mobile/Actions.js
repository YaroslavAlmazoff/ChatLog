...
const Actions = ({
  ...
  const openNotifications = async () => {
    const response = await api.get(`/api/checknotification/${auth.userId}`, {
      headers: {
  ...
  const createRoom = async () => {
    await api.get(`/api/create-room/${params.id}`, {
      headers: {
        Authorization: `Bearer ${auth.token}`,
      },
    });
    const response = await api.get(`/api/room-by-users/${params.id}`, {
      headers: {
        Authorization: `Bearer ${auth.token}`,
      },
    });
    ...
    <div className="user-mobile-actions">
      {isOwner ? (
        <div className="user-mobile-actions-self">
          <p className="user-add-foto-mobile" onClick={gotoCreatePostPage}>
            <img
          ...
          <p className="user-add-foto-mobile" onClick={gotoEdit}>
            <img
              src={require("./img/update.png")}
              width="10"
              alt="create post"
            />
            &nbsp;&nbsp;Обновить профиль
          </p>
          <p className="user-add-foto-mobile" onClick={openNotifications}>
            <img
        ...
        <div className="user-mobile-actions-self">
          <p className="user-add-foto-mobile" onClick={makeFriends}>
            <img
          ...
          <p className="user-add-foto-mobile" onClick={createRoom}>
            <img
              src={require("./img/messages.png")}
              alt="notifications"
              width="10"
            />
            &nbsp;&nbsp;Написать сообщение
