...
import usePosts from "../../hooks/usePhotos";
...
const Fotos = ({
  ...
  return (
    <div className="user-fotos-mobile">
        ...
        {isOwner ? (
          <div>
            ...
            <button
              ...
              className="user-add-foto-right"
            >
              Добавить фотографию
            </button>
            <button
              onClick={() =>
                sendFoto(file2, getCurrentDate, params, userFotos, setUserFotos)
              }
              className="user-add-foto-right"
            >
      ...
      {userFotos.map((el) => (
        <div
          className="foto-div-mobile"
          onClick={() => showFotography(el.imageUrl)}
          key={randomKey()}
        >
          <FotoItemMobile imageUrl={el.imageUrl} />
        </div>
      ))}
    </div>
  );
