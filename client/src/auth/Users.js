    ...
    return (
        <div className="users">
                ...
                {!users[0] ? <Loader ml={'0%'} /> : <div className="users-list">
                    {searchedUsers.map(el => <UserItem key={randomKey()} name={el.name} surname={el.surname} age={el.age} avatarUrl={el.avatarUrl} id={el._id} /*isFriends={el.isFriends} friendsButtonText={el.friendsButtonText}*/ />)}
                </div>}
                {/*<div className="users-ads">
