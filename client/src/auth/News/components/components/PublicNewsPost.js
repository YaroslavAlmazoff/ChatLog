...
const PublicNewsPost = ({id}) => {
    ...
    const [post, setPost] = useState({images: [], title:'',text:'',date:'', _id: '630cd7d5be8d2f7728d1e691'})
    ...
    const mark = async () => {
        ...
        await api.post(`/api/public/notify/${post._id}/${id}`, {type: LIKE_NOTIFICATION}, {headers: {
        ...
        if(like === require('../../../../img/blue-like.png')) {
            ...
            await api.get(`/api/public/likepost/${post._id}`, {headers: {
                Authorization: `Bearer ${auth.token}`
            }}) 
        }   
    } 
        ...
        <div className="user-post">
            {
                ...
                <>
                    ...
                    <h2 className={`user-post-title ${randomColor()} ${randomShadow()}`}>{post.title}</h2>
                    <p className={`user-post-text ${randomColor()} ${randomShadow()}`}>{post.text}</p>
                    ...
                    <p className="user-post-date">От {post.date}</p></> : <p style={{color: 'white'}}>{error}</p>}
                    <>
                    {
                    commentsDisplay && 
                    <div>
                        <PublicCommentField id={post._id} comments={comments} setComments={setComments} publicID={id} />
