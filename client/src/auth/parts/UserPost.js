...
const UserPost = ({post, isOwner, userPosts = [], setUserPosts = () => {}}) => {
    ...
    const mark = async () => {
        if(like === require('../../img/blue-like.png')) {
            ...
            setLike(require('../../img/red-like.png'))
            await api.post(`/api/like`, {id: post._id}, {headers: 
            ...
            await api.post(`/api/like`, {sub: true, id: post._id}, {headers: 
                {Authorization: `Bearer ${auth.token}`}
            })
    ...
    return (
        <div className="user-post">
            {isOwner && 
                <div className="user-post-delete"><span title="Удалить запись?" onClick={() => deletePost(post._id)} className="public-post-delete">&times;</span></div>
            }
            <p className={`user-post-text ${randomColor()} ${randomShadow()}`}>{post.title}</p>
            {post.images.length ? <img onLoad={() => setMainImageLoading(false)} className="user-post-image" src={image} alt="ad" style={!mainImageLoading ? {display: 'block'} : {display: 'none'}} /> : null}
                <div className="user-post-images">
                {post.images.length > 1 && post.images.map(el => <img 
                    style={process.env.REACT_APP_API_URL + '/articles/' + el === image ? {border: '1px solid rgb(0, 140, 255)'} : {border: 'none'}} 
                    onClick={() => setImage(process.env.REACT_APP_API_URL + '/articles/' + el)} 
            ...
            <>
                ...
                commentsDisplay && 
                <div>
                    <CommentField id={post._id} comments={comments} setComments={setComments} />
