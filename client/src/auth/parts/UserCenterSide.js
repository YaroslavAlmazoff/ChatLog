...
const UserCenterSide = ({
    ...
    return (
        <div className="user-center-side">
                    {isOwner ? <button className="button margin-top-bottom" onClick={gotoCreatePostPage}>Создать новую запись</button> : <></>}
                    {isOwner && isAdmin ? <button className="button margin-top-bottom" onClick={gotoAdmin}>Кабинет администратора</button> : <></>}
