...
const UserItem = ({name, surname, age, avatarUrl, id/*, isFriends, friendsButtonText*/}) => {
    ...
    const createRoom = async (e) => {
        ...
        if(response.data.room) {
            ...
            const response1 = await api.get(`/api/createroom/${id}`, {headers: {
                Authorization: `Bearer ${auth.token}`
            }})
            console.log(response1)
            const response2 = await api.get(`/api/getroom/${id}`, {headers: {
                Authorization: `Bearer ${auth.token}`
            }})
    ...
    // }
    return (
        <div onClick={() => gotoUser(id)} className={`user-item ${randomBlockShadow()}`}>
            <div className="user-item-right-side">
                <div><img className={`user-item-img ${randomBlockShadow()}`} src={process.env.REACT_APP_API_URL + '/useravatars/' + avatarUrl} alt="user" /></div>
                <div className="user-item-info">
                    <h3 className={`user-item-name ${randomColor()} ${randomShadow()}`}>{name} {surname}</h3>
            ...
            <div>
                ...
                ? <div className="user-item-actions">
                    <button onClick={(e) => createRoom(e)} className="button" style={{marginBottom: '5px'}}>Написать сообщение</button>
                    {/* {!isFriends ? <button onClick={!isFriends ? (e) => makeFriends(e) : (e) => noop(e)} className="button" >{friendsButtonText}</button> : <></>} */}
                </div>
                : <></>
                }
            </div>
