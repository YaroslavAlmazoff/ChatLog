...
const UserNav = ({user, isOwner, noticeDisplay, setNoticeDisplay, noticeText, noticeRef}) => {
    ...
    return (
        <div className="user-nav block" style={user.bannerUrl === "banner.jpg" ? {backgroundColor: 'transparent'} : {backgroundImage: `url(${process.env.REACT_APP_API_URL + '/userbanners/' + user.bannerUrl})`}}>
            <Notice noticeDisplay={noticeDisplay} setNoticeDisplay={setNoticeDisplay} noticeText={noticeText} noticeRef={noticeRef} />
            <img className="user-avatar block" src={process.env.REACT_APP_API_URL + '/useravatars/' + user.avatarUrl} alt="useravatar" />
            <div className="banner">
                <div className="user-nav-info">
                    <h2 className={`user-name ${randomColor()} navy-text-glow`}>{user.name} {user.surname}</h2>
                ...
                {user.age ? <h2 className="user-age">Дата рождения: {normalizeBirthDate(`${user.age}`)}</h2> : null}
                    {isOwner ? <button onClick={gotoEdit} className="button">Редактировать профиль</button> : <></>}
                </div>
            </div>
        </div>
    )
