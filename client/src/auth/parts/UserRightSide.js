        ...
        <div className="user-left-side">
                    {isOwner ? <div>
                        <div ref={notificationRef}>
                            {notifications.length !== 0 ? <div>{!notifications[notifications.length - 1].checked ? <div style={{width: '60px', backgroundColor: 'red', borderRadius: '15px', marginBottom: '-30px', zIndex: '10', color: 'white'}}><p>Новые</p></div> : <></>}</div> : <></>}
                        ...
                        <button onClick={(e) => emitOpen2(e)} className="user-add-foto-right button">Добавить фотографию</button>
                        <button onClick={() => sendFoto(file2, userFotos, setUserFotos)} className="user-add-foto-right button">Отправить фотографию</button>
                    ...
                    {userFotos.map(el => <div className="foto-div" key={randomKey()}><FotoItem imageUrl={el.imageUrl} /></div>)}
                    </div>
