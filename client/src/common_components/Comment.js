...
const Comment = ({item}) => {
    ...
    const like = async () => {
        if(!isLike && localStorage.getItem(item._id) !== auth.userId) {
            ...
            setLikesCount(likesCount - 1)
            setIsLike(false)
            const response = await api.get(`/api/userpost/dislikecomment/${item._id}`)
            console.log(response)
  ...
  return (
    <div className="videohost-comment-item">
        <div className="videohost-comment-item-avatar">
            ...
            <div onClick={like}>
                <span onClick={gotoUser} className="videohost-comment-item-likes-count">{likesCount}</span>
        ...
        <div className="videohost-comment-item-main">
            <div className="videohost-comment-item-head">
                <span className="videohost-comment-item-author" style={{cursor: 'pointer'}}>{user.name} {user.surname}</span>
