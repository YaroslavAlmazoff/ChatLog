...
const ChatRoom = () => {
    ...
    const removeDublicates = () => {
        setMessages(prev => prev.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i))
    } 

    const subscribe = async () => {
        if(!params.id) return
        console.log('connection')
        const eventSource = new EventSource(`https://chatlog.ru/api/chatconnect/${params.id}`)
    ...
    useEffect(() => {
        const getMessages = async () => {
            ...
            setMessages(response.data.messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i))
            roomRef.current.scrollTop = roomRef.current.scrollHeight;
            setLoading(false)
        }
        getMessages()
    ...
    useEffect(() => {
        const dialog = async () => {
            setLoading(true)
            const response = await api.get(`/api/chatroombyid/${params.id}`, {headers: {
    ...
    const sendMessage = async () => {
        if(!currentMessage._id) {
            ...
            const message = {
                ...
                date,
                file: imagePreviewUrl1,
                videoFile: videoPreviewUrl,
                audioFile,
                isFile: !!localStorage.getItem('file-link'),
            }
            await api.post(`/api/lastchatmessage/${params.id}`, {lastMessage: messageRef.current.value})
            messageRef.current.value = ''
            await api.post(`/api/new-chatmessages/${params.id}`, {message}, {
                headers: {
                    Authorization: `Bearer ${auth.token}`
                }
            })
        } else {
            await api.post(`/api/editchatmessage/${currentMessage._id}`, {message: messageRef.current.value})
            messageRef.current.value = ''
    ...
    const deleteMessage = async () => {
        const response = await api.delete(`/api/deletechatmessage/${currentMessage._id}`)
    ...
    useEffect(() => {
        ...
        if(messages[messages.length - 1].user) {
            console.log(auth.userId, messages[messages.length - 1], messages[messages.length - 1].user)
            console.log(auth.userId.toString() != messages[messages.length - 1].user.toString())
            if(auth.userId.toString() != messages[messages.length - 1].user.toString()) {
                console.log("what")
                readMessage()
            }
        }
    ...
    return (
            ...
            <div ref={roomRef} className="room-window block" style={{backgroundImage: `url(${process.env.REACT_APP_API_URL + '/roombackgrounds/' + bg})`}}>
                <div className="room-head">
                    <span onClick={!isCheckMembers ? openMembers : closeMembers } className={`${randomColor()} ${randomShadow()}`} style={{cursor: 'pointer'}}>{membersText}</span> | 
                    <button className="button" style={{pading: '8px'}} onClick={!isAddMembers ? openAddMembers : closeAddMembers}>{buttonText}</button></div>
                    ...
                    <div className="messages">
                        ...
                        <img onClick={(e) => emitOpenBg(e)} className="upload-bg" src={require('../../img/upload-bg.png')} alt="" title="Установить фон для переписки" />
                    </div>
                </div>
            <div className="message-actions">
                <img id="stop" className="upload-message-image" style={isRecording ? {display: 'inline-block'} : {display: 'none'}} src={require(`../../img/stop.png`)} alt='img'/>
                ...
                {!isRecording ? <img id="start" className="upload-message-image" src={require(`../../img/recording.png`)} alt='img'/> : null}
                <img onClick={sendMessage} className="upload-message-image" src={require(`../../img/send.png`)} alt='img'/>
                <input onChange={(e) => getFile(e)} ref={fileRef} type="file"  accept=".jpg,.jpeg,.png,.gif" />
                <input onChange={(e) => getFileVideo(e)} ref={fileRefVideo} type="file" accept=".mp4" />
                <input onChange={(e) => getFileBg(e)} ref={bgRef} type="file" accept=".jpg,.png,.jpeg,.gif" />
            </div>
