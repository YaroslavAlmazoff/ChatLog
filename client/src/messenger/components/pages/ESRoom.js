...
export const ESRoom = () => { 
    ...
    const subscribe = async () => {
        ...
        const eventSource = new EventSource(`https://chatlog.ru/api/connect/${params.id}`)
    ...
    const sendMessage = async () => {
        if(!currentMessage._id) {
            ...
            await api.post(`/api/new-messages/${params.id}`, 
                {
                    message: text,
                    date: getCurrentDate(),
                    file: imageUrl,
    ...
    return (
            ...
            <div ref={roomRef} className="room-window block" style={{backgroundImage: `url(${process.env.REACT_APP_API_URL + '/roombackgrounds/' + bg})`}}>
                <div className="room-head">
                    <div className="room-message-actions" style={{display: messageActionsDisplay}}>
                        <span onClick={editMessage} className="room-message-action1" style={{color: 'rgb(0, 140, 255)'}}>Редактировать</span>
                        <span onClick={deleteMessage} className="room-message-action2" style={{color: 'red'}}>Удалить</span>
                    </div>
                    <span className={`${randomColor()} ${randomShadow()}`}>{penFriend} | В сети {fullPenFriend.lastVisit}</span>
                </div>
            ...
            <div className="message-actions">
                ...
                <input ref={messageRef} type="text" className={!isRecording ? "message-input input" : "message-input-recording"} placeholder={!isRecording ? "Напишите сообщение..." : "Говорите..."} />
                ...
                <input onChange={(e) => getFileBg(e)} ref={bgRef} type="file" accept=".jpg,.jpeg,.png,.gif" />
            </div>
