...
const Head = ({pub, isAdmin, subscribers, setSubscribers, isNotifications, setIsNotifications}) => {
    ...
    useEffect(() => {
        ...
        const getIsSubscriber = async () => {
            const response = await api.get(`/api/public/issubscriber/${pub._id}`, {headers: {
    ...
    const subscribe = async () => {
        const response = await api.get(`/api/public/subscribe/${pub._id}`, {headers: {
            Authorization: `Bearer ${auth.token}`
        }})
        if(response.data.isSubscriber) {
    ...
    return (
        <div className="public-head" style={{backgroundImage: `url(${process.env.REACT_APP_API_URL + `/publicbanners/` + pub.bannerUrl})`}}>
            <div className="public-info">
                ...
                <div className="public-bottom-info">
                    ...
                    {!isAdmin ? <button onClick={subscribe} className="public-subscribe">{buttonText}</button>: <div></div>}
                </div>
            </div>
            {isAdmin ? <div className="public-actions">
                ...
                <div onClick={edit} className="public-action" style={{marginLeft: '-10px'}}>
                    <img className="public-action-img" src={require('../../img/pencil.png')} alt="edit" />
                    <p className="public-action-name">Редактировать</p>
                </div>
                <div onClick={notifications} className="public-action" style={{marginLeft: '-8px'}}>
