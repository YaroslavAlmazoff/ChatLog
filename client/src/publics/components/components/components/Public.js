...
const Public = ({item}) => {
    ...
    useEffect(() => {
        ...
        const getIsSubscriber = async () => {
            const response = await api.get(`/api/public/issubscriber/${item._id}`, {headers: {
    ...
    return (
        <div className={`public-item ${randomBlockShadow()}`} onClick={gotoPublic}>
            <div className="public-item-info">
                {item.avatarUrl 
                ? <img className={`public-item-avatar ${randomBlockShadow()}`} src={process.env.REACT_APP_API_URL + `/publicavatars/` + item.avatarUrl} alt="public avatar" />
                : <img className={`public-item-avatar ${randomBlockShadow()}`} src={require('../../../img/group.png')} alt="group" />
                }
                
                <p className={`public-item-name ${randomColor()} ${randomShadow()}`}>{item.name}</p>
            </div>
            <button onClick={(e) => subscribe(e)} className="button">{buttonText}</button>
        </div>
    )
}
