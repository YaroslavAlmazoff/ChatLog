...
const EditPage = () => {
    ...
    const editPublic = async () => {
        ...
        const response = await api.post(`/api/public/edit/${params.id}`, formData, {headers: 
    ...
    return (
        <div className="create-public">
            {!loading ? <div className="create-public-form">
                ...
                <div className='create-public-upload-field'>
                    ...
                    {avatarOpened ? <ImagePreview imageUrl={imagePreviewUrl1}  /> : <>{pub.avatarUrl ? <img width="150" src={process.env.REACT_APP_API_URL + '/publicavatars/' + pub.avatarUrl} /> : null}</>}
                </div>
                <div className='create-public-upload-field'>
                    <button onClick={(e) => emitOpen2(e)} className='create-public-button'>Выберите баннер</button>
                    {bannerOpened ? <ImagePreview imageUrl={imagePreviewUrl2}  /> : <>{pub.bannerUrl ? <img width="150" src={process.env.REACT_APP_API_URL + '/publicbanners/' + pub.bannerUrl} /> : null}</>}
                </div>
                <button className='create-public-button' onClick={editPublic}>Редактировать</button>
