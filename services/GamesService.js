...
class GamesService {
    ...
    async updateGame(req, res) {
        ...
        if(req.files.preview) {
            await Game.findByIdAndUpdate(req.params.id, {name, description, version, previewUrl, downloadUrl})
            FileService.insertGameSourсe(req.files.source, downloadUrl)
            FileService.insertGamePreview(req.files.preview, previewUrl)
        } else {
            await Game.findByIdAndUpdate(req.params.id, {name, description, version, downloadUrl})
            FileService.insertGameSourсe(req.files.source, downloadUrl)
        }
    ...
    async rate(req, res) {
        ...
        await Game.findByIdAndUpdate(req.params.id, {rating: itog, summRating, marks: marks + 1})
        res.json({msg: 'success'})
    }
    async comment(req, res) {
        const {comment, date} = req.body
        GameComment.create({comment, date, user: req.user.userId, game: req.params.id})
        .then(async () => {
