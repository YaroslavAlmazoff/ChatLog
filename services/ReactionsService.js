...
class ReactionService {
    ...
    async checkLike(req, res) {
        const like = await Like.findOne({user: req.user.userId, post: req.params.id})
        if(like != null) {
            res.json({liked: true})
        } else {
    ...
    async like(req, res) {
        try {
            ...
            if(like) {
                ...
                await UserPost.findByIdAndUpdate(id, {likes: newLikes, likers})
                ...
                await NotificationService.create(userid, needArticle.user, text, 'like', 'article', id)
    ...
    async commentUserPost(req, res) {
        ...
        Comment.create({comment: text, date, user: req.user.userId, articleID: req.params.id})
        .then(async comment => {
            ...
            const text = `Вашу запись прокомментировал пользователь ${user.name} ${user.surname}: ${text}`
            await NotificationService.create(req.user.userId, post.user, text, 'comment', 'article', '')
