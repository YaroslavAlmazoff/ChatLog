...
const removeDublicates = async (req) => {
    ...
    if(message) {
        const sameMessages = await Message.find({message: message.message, old: message.message, message: message.old, old: message.old, date: message.date, room: message.room})
...
router.get('/connect/:id', async (req, res) => {
    ...
    emitter.on('newMessage', async (message, req) => {
        console.log("ploho")
        Message.findOne({message: message.message, date: message.date, room: message.room})
        .then(async data => {
            if(data) {
                console.log("уже существует")
                await removeDublicates(req)
                const messages = await Message.find({room: message.room})
                const filtered = messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i)
                ...
                if(message.file) {
                    ...
                    Message.create({...message, imageUrl: filename})
                    .then(async () => {
                        await removeDublicates(req)
                        const messages = await Message.find({room: message.room})
                        const filtered = messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i)
                    ...
                    if(req.files.file) {
                        ...
                        Message.create({...message, imageUrl: filename1})
                        .then(async () => {
                            await removeDublicates(req)
                            const messages = await Message.find({room: message.room})
                            const filtered = messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i)
                        ...
                        await FileService.insertMessageVideo(req.files.videoFile, filename1)
                        Message.create({...message, videoUrl: filename1})
                        .then(async () => {
                            await removeDublicates(req)
                            const messages = await Message.find({room: message.room})
                            const filtered = messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i)
                        ...
                        await FileService.insertMessageAudio(req.files.audioFile, filename1)
                        Message.create({...message, audioUrl: filename1})
                        .then(async () => {
                            await removeDublicates(req)
                            const messages = await Message.find({room: message.room})
                            const filtered = messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i)
                    ...
                    ImageService.saveVideoBase64(message.videoFile, filename, 'messagevideos')
                    Message.create({...message, videoUrl: filename})
                    .then(async () => {
                        await removeDublicates(req)
                        const messages = await Message.find({room: message.room})
                        const filtered = messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i)
                    ...
                    ImageService.saveAudioBase64(message.audioFile, filename, 'messageaudios')
                    Message.create({...message, audioUrl: filename})
                    .then(async () => {
                        await removeDublicates(req)
                        const messages = await Message.find({room: message.room})
                        const filtered = messages.filter((v,i,a)=>a.findIndex(t=>(t.message === v.message && t.date===v.date))===i)
                    ...
                    Message.create({...message, isNotReaded: true})
                    .then(async () => {
...
router.post('/new-messages/:id', auth, (async (req, res) => {
    ...
    message.isFile = message.isFile == "true" || message.isFile == true ? true : false
